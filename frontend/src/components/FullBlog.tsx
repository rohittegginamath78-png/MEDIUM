...
export const FullBlog = ({ blog }: FullBlogProps) => {
  const authorName = blog?.author.name ?? "Jese Leos";
  const publishedAt = blog?.publishedAt
    ...
    : "Feb. 8, 2022";
  const title = blog?.title ?? "Best practices for successful prototypes";
  const content =
    blog?.content ??
          ...
          <header className="mb-4 lg:mb-6 not-format">
              <address className="flex items-center mb-6 not-italic">
                  <div className="inline-flex items-center mr-3 text-sm text-gray-900 dark:text-white">
                      <img className="mr-4 w-16 h-16 rounded-full" src="https://flowbite.com/docs/images/people/profile-picture-2.jpg" alt={authorName} />
                      <div>
                          <a href="#" rel="author" className="text-xl font-bold text-gray-900 dark:text-white">{authorName}</a>
                          <p className="text-base text-gray-500 dark:text-gray-400">Graphic Designer, educator & CEO Flowbite</p>
                          <p className="text-base text-gray-500 dark:text-gray-400"><time dateTime={blog?.publishedAt ?? "2022-02-08"} title={publishedAt}>{publishedAt}</time></p>
                      </div>
                  </div>
              </address>
